...
- 含义相近的词，向量也很接近——“猫”和”狗”比”猫”和”桌子”更像
...
示意图里每个向量只有 4 个数字，但真正的大语言模型用的向量要长得多：
...
> 经常出现在相似位置的词，向量就会变得相似——词嵌入不是人工设计的，是大语言模型自己”学”出来的。
...
大语言模型拿每个字的 Query 去匹配所有字的 Key，找到最相关的字，再把它们的 Value 组合起来。就像上课举手提问（Query），看看同学们的名牌（Key），然后听对的人回答（Value）。

----

## Q、K、V 长什么样？

想象每个字都有一张“成绩单”，评分项代表不同方面的含义。

- **Q（查询）** = “我想找什么样的搭档”的成绩单
- **K（键）** = “我自己能提供什么”的成绩单
- 成绩单有几项评分，就叫 **维度 d**

真正的大语言模型用 d=512，这里我们简化到 **d=4** ——4 项评分分别代表：谁在做、做什么、在哪里、什么东西。
...
> 注意：这些数字是为了方便演示而简化的。真正的大语言模型会在训练中 **自动学会** 每个字的 Q 和 K。
...
就像配对交朋友：共同爱好越多（分数都高的项越多），总分就越大——越合拍！来算“猫”和每个字有多合拍：
...
**第 3 步：按比例混合** → 新“猫” = 15%×猫 + **69%×坐** + 15%×垫

结果：“猫”的新表示里，69% 来自“坐”——因为“猫”最想知道自己在干什么！
...
饼图几乎被“坐”一个字独占了！其他字的信息完全被忽略——大语言模型变成了“只看一个字”。
...
这就是“从分数到百分比”里那一步“缩一缩”的原因——**防止大语言模型只盯着一个字看** 。
...
1. **多头注意力** ：每个字同时扫一眼所有字，多双眼睛捕捉不同关系
2. **前馈网络 (Feed-Forward Network)** ：每个字根据收集到的线索，独立“想一想”，更新自己的理解
...
> 捷径连接 + 层归一化，合在一起就是 Transformer 架构图中绿色的“残差连接 + 层归一化”。
...
同一个词，最终的理解截然不同——这就是 12 层、24 层、甚至 96 层“变换”的力量。
...
每个位置 **只能看到前面的字** ，看不到后面的：

> 位置 1：“从” → 只看到自己
> 位置 2：“前” → 看到“从”
> 位置 3：“有” → 看到“从”“前”

为什么要这样？因为生成时后面的字还没写出来——不能偷看还不存在的答案！
