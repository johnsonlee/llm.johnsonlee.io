...
今天的大语言模型解决了这些问题。说到底，它就是一个巨大的 **数学函数** ：

- **输入** ：一段文字
- **输出** ：下一个词的概率
- **内部** ：几千亿个数字在做加减乘除

函数就是"给一组输入，算出一组输出"。大语言模型和数学课上的 y = f(x) 本质相同——只是维度天差地别。

----

## 从 y = f(x) 到大语言模型

![从二维函数到高维函数](src/zh/images/math-function.svg)

----

## 从文字到运算，三步走

函数只认数字，不认文字！要让大语言模型工作，需要三步：

1. **切碎** ：把文字拆成小片段（token）
2. **定位** ：给每个 token 算出一个坐标（向量），放进数学空间
3. **运算** ：根据上下文，动态调整每个 token 的坐标——让大语言模型真正“理解”语境

三步之后，大语言模型就能“理解”文字并给出回答。接下来一步步看！
...
*embedding 给了每个 token 一个固定坐标——但“苹果”在“吃苹果”和“苹果公司”里含义不同，怎么办？*
...
方法叫 **点积** ——把两张成绩单上的每一科分数对应相乘，再全部加起来。

强项越重合（同一科分数都高），总分就越大——越合拍！来算”猫”和每个 token 有多合拍：
