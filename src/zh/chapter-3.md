...
![缩放定律曲线](src/zh/images/scaling-curve.svg)
...
钱花对了，大语言模型稳步变大。但在变大的路上，出现了一些谁也没预料到的现象：
...
- 原来的评估方式是”全对才算对”——算错一步就是 0 分
- 小模型全错，大一点还是全错，突然有一天全对了—— **看起来像”开窍”**
- 换成”算对了几步”的评估方式，进步其实是 **循序渐进** 的
- 所谓的”突然涌现”，也许只是评估方式制造的错觉
...
不管涌现是不是幻觉，有一个现实问题绕不开：互联网虽大，数据量终究有限。当大语言模型的胃口超过了现有的数据量，该怎么办？

![数据瓶颈的三条出路](src/zh/images/data-wall-solutions.svg)
